...
@external(javascript, "../now_playing_ffi.mjs", "mountNowPlaying")
fn mount(root: Dynamic, inner_id: String) -> Nil

pub const element_name: String = "audio-player-now-playing"

const inner_id: String = "apww-now-playing-root"
...
fn init(_flags: Nil) -> #(Model, effect.Effect(Msg)) {
  #(
    Nil,
    effect.after_paint(fn(_dispatch, root) { mount(root, inner_id) }),
  )
}
